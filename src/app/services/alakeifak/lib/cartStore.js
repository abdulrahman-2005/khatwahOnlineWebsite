...
export const useCartStore = create((set, get) => ({
  ...
  initCart: (slug) => {
    try {
      ...
      if (saved) {
        ...
        set({ items: parsed.items || [], deliveryZone: parsed.deliveryZone || null, restaurantSlug: slug });
      } else {
        set({ items: [], deliveryZone: null, restaurantSlug: slug });
      }
    } catch {
      set({ items: [], deliveryZone: null, restaurantSlug: slug });
    }
  ...
  _persist: () => {
    const { items, deliveryZone, restaurantSlug } = get();
    if (restaurantSlug) {
      localStorage.setItem(
        `khatwah_alakeifak_cart_${restaurantSlug}`,
        JSON.stringify({ items, deliveryZone })
      );
    }
  },
